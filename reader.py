...
def readShortVideo(video_path, video_category, video_name, downsample_factor=12, rescale_factor=1):
    ...
    for frameIdx, frame in enumerate(videogen):
        if frameIdx % downsample_factor == 0:
            frame = skimage.transform.rescale(frame, [299/frame.shape[0], 299/frame.shape[1]], mode='constant', preserve_range=True, multichannel=True, anti_aliasing=True).astype(np.uint8)
            #frame = Image.fromarray(frame.astype('uint8'), 'RGB')
            #frame[:,:,0] = preprocessing.normalize(frame[:,:,0])
            #frame[:,:,1] = preprocessing.normalize(frame[:,:,1])
            #frame[:,:,2] = preprocessing.normalize(frame[:,:,2])
            frames.append(frame)
        else:
            continue
    #return frames
...
#     videos_list = getVideoList('./hw4_data/TrimmedVideos/label/gt_train.csv')
#     for key in videos_list:
#         print(key)
#     print(len(collections.Counter(videos_list['Video_category'])))